# app.py (VERSÃO COMPLETA E FINAL - CARREGAMENTO SOB DEMANDA)
...
# Importe os layouts e a função de carregar dados com cache
import login
import menu 
import operacao
import desempenho
...
def preparar_dataframe_completo():
    """
    Função central que chama a função com cache para carregar e depois processa os dados.
    """
    print("INFO: Chamando a função para obter/carregar dados...")
    df = carregar_dados("BaseReceber2025", "BaseReceber")
    
    if 'Erro' in df.columns:
        print(f"ERRO: Erro retornado pela função de carregamento de dados: {df['Erro'].iloc[0]}")
        return df
    ...
    print("INFO: Processamento do DataFrame concluído.")
    return df
...
def display_page(pathname):
    print(f"INFO: Navegando para: '{pathname}'")
    ...
    if pathname == '/desempenho': return desempenho.layout
    if pathname in ['/comercial', '/evolucao']:
        return html.Div("Página em construção...", style={'textAlign': 'center', 'padding': '50px'})
...
def fazer_login(n_clicks_btn, n_submit_senha, usuario, senha):
    ...
    if usuario == 'admin' and senha == '123':
        return '/menu', ""
    return no_update, "Usuário ou Senha incorreta"

# 3. CALLBACKS DE PÁGINA (CARREGAM DADOS SOB DEMANDA)
...
def popular_competencia(pathname):
    ...
    competencias_sorted = sorted(competencias, key=lambda x: (str(x).split('/')[1], str(x).split('/')[0]), reverse=True)
...
def gerar_kpis_e_cards(competencia, pathname):
    if not competencia or not (pathname.startswith('/faturamento') or pathname.startswith('/operacao/')):
        return no_update, no_update, no_update, no_update
    
    df_full = preparar_dataframe_completo()
    if 'Erro' in df_full.columns: return html.Div(df_full['Erro'].iloc[0]), [], "Erro", "/"
    ...
    gauge_fig = go.Figure(go.Indicator(
        ...
        gauge={'axis': {'range': [0, meta_clientes * 1.5 if meta_clientes > 0 else 10]}, 'bar': {'color': "#007bff"},
...
@app.callback(Output('clientes-faturados-container', 'children'), [Input('competencia-store', 'data')], [State('url', 'pathname')])
def gerar_lista_faturas_tabela(competencia, pathname):
    if not competencia or not (pathname.startswith('/faturamento') or pathname.startswith('/operacao/')):
        return no_update
    
    df_full = preparar_dataframe_completo()
    if 'Erro' in df_full.columns: return html.Div(df_full['Erro'].iloc[0])
...
@app.callback(Output('ranking-container', 'children'), [Input('competencia-store', 'data')], [State('url', 'pathname')])
...
@app.callback(Output('analise-avancada-container', 'children'), [Input('competencia-store', 'data')], [State('url', 'pathname')])
...
@app.callback(Output('faturamento-diario-container', 'children'), [Input('competencia-store', 'data')], [State('url', 'pathname')])
...
@app.callback(Output('evolucao-anual-container', 'children'), [Input('competencia-store', 'data')], [State('url', 'pathname')])
...
@app.callback(
    ...
    Input('url', 'pathname') # Acionado quando a página carrega
)
def atualizar_projecao_recebiveis(pathname):
...
def atualizar_recebimentos_cobranca(pathname):
    if pathname != '/cobranca': return no_update
...
# --- CALLBACK PARA O BOTÃO DE ATUALIZAÇÃO ---
@app.callback(
...
def handle_refresh_button(n_clicks):
    ...
    # A função carregar_dados é importada de sheets_api.py
    # e tem o decorador @lru_cache.
    # .cache_clear() limpa o resultado armazenado.
    carregar_dados.cache_clear()
    print("INFO: Cache de dados do servidor limpo. A próxima página que precisar de dados irá recarregá-los.")
    
    return f"Cache limpo às {datetime.now().strftime('%H:%M:%S')}. Recarregue a página para ver os novos dados."
